...
@OpenAPIDefinition(
        info = @Info(title = "Meeting Web",
                description = "Meeting api명세",
                version = "v1"))
...
public class SwaggerConfig {
    ...
    private void generateErrorResponseExample(Operation operation, Class<? extends ErrorCode> type) {
        ...
        ErrorCode[] commonErrorCodes = CommonError.values();
        ErrorCode[] allErrorCodes = Stream.concat(Arrays.stream(errorCodes), Arrays.stream(commonErrorCodes))
        ...
        Map<Integer, List<ExampleHolder>> statusWithExampleHolders =
                Arrays.stream(allErrorCodes)
                        ...
                        .collect(Collectors.groupingBy(ExampleHolder::getHttpCode));
